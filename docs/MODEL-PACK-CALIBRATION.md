...
| gemma3      | `PROVISIONAL`          | OI2 loader+forward (SWA + final soft-cap + sandwich norms + token-embd-scale) landed. Calibration **blocker patch landed in working tree** via `shannon-prime-engine/patches/ggml-cuda-getrows-kquant.patch` (applied to `vendor/ggml/src/ggml-cuda/getrows.cu`) — adds K-quant / IQ* dispatch to the CUDA `get_rows` kernel, reusing the per-row dequantizers already in `convert.cu`. Needs engine rebuild on Windows + PPL run on `lmstudio-community/gemma-3-12b-it-Q3_K_L.gguf` to land a ledger row. Same fix also unblocks `phi-4-Q4_K_M` and `Phi-3.1-mini-128k-Q4_K_M` (K-quant embed tables). |
