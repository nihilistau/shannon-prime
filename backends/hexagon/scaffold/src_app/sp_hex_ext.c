...
// ============================================================================
// Phase 2.0 weight streaming probe — sp_hex_weight_stream_bench
// ============================================================================
//
// Producer/consumer ring across the rpcmem boundary:
//   ARM I/O thread: pread() from a backing weight file into the next slot
//                   in the rpcmem tile pool, set flags[slot] = READY.
//   cDSP consumer:  inside one long FastRPC call, polls flags[slot] for
//                   READY, decompresses + dot-products, sets flags[slot]
//                   = CONSUMED. ARM thread refills the slot.
//
// We measure throughput (bytes/sec end-to-end) and the wait/compute split
// reported by the DSP via qtimer. Success = wait > 0 (DSP waiting on UFS,
// thesis confirmed) AND throughput approaches UFS line speed (~1.2 GB/s
// realistic on UFS 3.1 of S22U).
...
typedef struct {
    ...
    int                n_slots;
    int                n_tiles_total;
    unsigned char     *tile_pool;       // rpcmem
    volatile uint32_t *flags;            // rpcmem
    volatile int       producer_done;
    volatile int       producer_err;
} ws_producer_ctx_t;

static void *ws_producer_thread(void *arg) {
    ws_producer_ctx_t *ctx = (ws_producer_ctx_t *)arg;
    long long offset = 0;
    for (int t = 0; t < ctx->n_tiles_total; ++t) {
        const int slot = t % ctx->n_slots;
        // Wait for slot to be EMPTY or CONSUMED (i.e., not READY).
        // Tight spin — slots empty quickly relative to UFS read time.
        while (__atomic_load_n(&ctx->flags[slot], __ATOMIC_ACQUIRE) == SP_TILE_READY) {
            // yield could go here; for the probe we want to measure
            // the ceiling, so spin
        }
        // pread the next tile bytes from the backing file.
        unsigned char *dst = ctx->tile_pool + (size_t)slot * (size_t)ctx->tile_bytes;
        ssize_t n = pread(ctx->fd, dst, (size_t)ctx->tile_bytes, offset);
        if (n != ctx->tile_bytes) {
            // On short read at EOF, wrap: the bench is throughput-only,
            // so we cycle the file to keep the DSP fed.
            if (n < 0) {
            ...
            offset = 0;
            ssize_t n2 = pread(ctx->fd, dst, (size_t)ctx->tile_bytes, offset);
            if (n2 != ctx->tile_bytes) {
                ctx->producer_err = -2;
                ctx->producer_done = 1;
                return NULL;
            }
        ...
        offset += n;
        if (offset >= ctx->file_bytes) offset = 0;  // wrap

        // Release the tile to the consumer.
        __atomic_store_n(&ctx->flags[slot], SP_TILE_READY, __ATOMIC_RELEASE);
    }
    ctx->producer_done = 1;
    return NULL;
}

static int ws_create_or_check_backing_file(const char *path, long long target_bytes) {
    struct stat st;
    if (stat(path, &st) == 0 && st.st_size >= target_bytes) {
        return 0;  // exists and big enough
    }
    printf("[wstream] creating backing weight file: %s (%lld bytes)\n",
           path, target_bytes);
    int fd = open(path, O_WRONLY | O_CREAT | O_TRUNC, 0644);
    if (fd < 0) {
        printf("[wstream] open(%s, O_CREAT) failed: %s\n", path, strerror(errno));
        return -1;
    }
    // Fill with deterministic SP-compressed bytes so the DSP's
    // band_dequantize doesn't trip on garbage. Use repeating pattern
    // sized to the smallest band record.
    const int chunk_size = 256 * 1024;  // 256 KB write chunks
    unsigned char *buf = (unsigned char *)malloc(chunk_size);
    if (!buf) { close(fd); return -1; }
    // Seed with structured pattern: each byte is (i ^ (i >> 7)) & 0xff.
    // This produces distinguishable values that the dequantize path
    // can decode without overflowing accumulators.
    long long written = 0;
    while (written < target_bytes) {
        for (int i = 0; i < chunk_size; ++i) {
            buf[i] = (unsigned char)((written + i) ^ ((written + i) >> 7));
        }
        long long to_write = target_bytes - written;
        if (to_write > chunk_size) to_write = chunk_size;
        ssize_t n = write(fd, buf, (size_t)to_write);
        if (n != to_write) {
            printf("[wstream] short write at %lld\n", written);
            free(buf);
            close(fd);
            return -1;
        }
    ...
    free(buf);
    close(fd);
    printf("[wstream] backing file ready (%lld bytes)\n", target_bytes);
    return 0;
}
...
int sp_hex_weight_stream_bench(const char *weight_file_path,
                                ...
                                int head_dim) {
    int rc = 0;
    if (tile_bytes <= 0) tile_bytes = 16 * 1024;
    if (n_slots   <= 0) n_slots   = 4;
    if (head_dim  <= 0) head_dim  = 128;

    printf("\n[wstream] === Phase 2.0 weight-streaming probe ===\n");
    printf("[wstream] file=%s target=%lld bytes  tile=%d  slots=%d  hd=%d\n",
           weight_file_path, file_bytes_target, tile_bytes, n_slots, head_dim);

    // Round file to whole tiles.
    long long tile_count_total = file_bytes_target / tile_bytes;
    if (tile_count_total < (long long)n_slots * 4) {
        printf("[wstream] WARN: tile_count_total=%lld < 4*n_slots; bumping target\n",
               tile_count_total);
        tile_count_total = (long long)n_slots * 4;
        file_bytes_target = tile_count_total * tile_bytes;
    }

    // Need backing file at least as big as one full ring's worth.
    long long backing_needed = (long long)n_slots * tile_bytes * 4;
    if (file_bytes_target < backing_needed) backing_needed = file_bytes_target;
    if (ws_create_or_check_backing_file(weight_file_path, backing_needed) != 0) {
        return 1;
    }

    int fd = open(weight_file_path, O_RDONLY);
    if (fd < 0) {
        printf("[wstream] open RO %s failed: %s\n", weight_file_path, strerror(errno));
        return 1;
    }

    // Allocate rpcmem tile pool + flags + activation.
    int heapid = RPCMEM_HEAP_ID_SYSTEM;
    long long pool_bytes = (long long)n_slots * tile_bytes;
    unsigned char *tile_pool = (unsigned char *)rpcmem_alloc(heapid,
                                    RPCMEM_DEFAULT_FLAGS, (int)pool_bytes);
    uint32_t *flags = (uint32_t *)rpcmem_alloc(heapid, RPCMEM_DEFAULT_FLAGS,
                                    sizeof(uint32_t) * n_slots);
    float *activation = (float *)rpcmem_alloc(heapid, RPCMEM_DEFAULT_FLAGS,
                                    sizeof(float) * head_dim);
    long long *stats = (long long *)rpcmem_alloc(heapid, RPCMEM_DEFAULT_FLAGS,
                                    sizeof(long long) * 8);
    if (!tile_pool || !flags || !activation || !stats) {
        printf("[wstream] rpcmem_alloc failed\n");
        rc = 1; goto cleanup;
    }
    memset(tile_pool, 0, (size_t)pool_bytes);
    memset((void *)flags, 0, sizeof(uint32_t) * n_slots);
    memset(stats, 0, sizeof(long long) * 8);
    for (int i = 0; i < head_dim; ++i) activation[i] = 1.0f / (float)head_dim;

    // Open FastRPC session.
    remote_handle64 h = (remote_handle64)-1;
    rc = sp_hex_open(sp_hex_URI CDSP_DOMAIN, &h);
    if (rc != 0) {
        printf("[wstream] sp_hex_open failed 0x%x\n", rc);
        goto cleanup;
    }

    // Spawn producer thread — must be running before we make the
    // (synchronous, blocking) DSP call.
    ws_producer_ctx_t prod;
    memset(&prod, 0, sizeof(prod));
    prod.fd            = fd;
    prod.file_bytes    = backing_needed;
    prod.tile_bytes    = tile_bytes;
    prod.n_slots       = n_slots;
    prod.n_tiles_total = (int)tile_count_total;
    prod.tile_pool     = tile_pool;
    prod.flags         = flags;
    ...
    pthread_t prod_tid;
    if (pthread_create(&prod_tid, NULL, ws_producer_thread, &prod) != 0) {
        printf("[wstream] pthread_create failed\n");
        rc = 1; goto close_h;
    }

    // Drive DSP. This call blocks until DSP finishes all n_tiles_total tiles.
    rc = sp_hex_weight_stream_session(h,
            (int)tile_count_total, tile_bytes, n_slots, head_dim,
            activation, head_dim,
            tile_pool, (int)pool_bytes,
            (unsigned int *)flags, n_slots,
            stats, 8);
    ...
    if (rc != 0) {
        printf("[wstream] sp_hex_weight_stream_session rc=0x%x\n", rc);
        goto close_h;
    }
    if (prod.producer_err) {
        printf("[wstream] producer thread error %d\n", prod.producer_err);
        rc = 1; goto close_h;
    }

    long long bytes  = stats[0];
    long long tot_pc = stats[1];
    long long wait_pc= stats[2];
    long long comp_pc= stats[3];

    double secs = (double)(t1.tv_sec - t0.tv_sec) +
                  (double)(t1.tv_usec - t0.tv_usec) * 1e-6;
    double mbps = (double)bytes / (1024.0 * 1024.0) / secs;

    double wait_pct = (tot_pc > 0) ? 100.0 * (double)wait_pc / (double)tot_pc : 0.0;
    double comp_pct = (tot_pc > 0) ? 100.0 * (double)comp_pc / (double)tot_pc : 0.0;

    printf("\n[wstream] === RESULTS ===\n");
    printf("[wstream] tiles=%lld  bytes=%lld  wall=%.3fs  throughput=%.1f MB/s\n",
           tile_count_total, bytes, secs, mbps);
    printf("[wstream] DSP pcycles: total=%lld  wait=%lld (%.1f%%)  compute=%lld (%.1f%%)\n",
           tot_pc, wait_pc, wait_pct, comp_pc, comp_pct);
    if (wait_pct > 5.0) {
        printf("[wstream] THESIS HOLDS: DSP wait > 5%% — UFS is the bottleneck, "
               "compute is effectively free.\n");
    } else {
        printf("[wstream] thesis WEAK: DSP wait < 5%% — compute is the bottleneck "
               "at this tile size; need faster kernel or different tiling.\n");
    }
...
cleanup:
    if (fd >= 0)      close(fd);
    if (tile_pool)    rpcmem_free(tile_pool);
    if (flags)        rpcmem_free((void *)flags);
    if (activation)   rpcmem_free(activation);
    if (stats)        rpcmem_free(stats);
    return rc;
}
#endif  // SP_HEXAGON_FASTRPC
